...
class Mistroll_Cats2(ShopBenchBaseModel):
    def __init__(self):
        random.seed(AICROWD_RUN_SEED)

        self.tokenizer = AutoTokenizer.from_pretrained('./models/FusionNet/', trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained('./models/FusionNet/', device_map='auto',
        ...
        if is_multiple_choice:
            ...
            "Select the category") in prompt:  # task 2
                ...
                prompt_edit = ("You will be given a product. Answer the given question based on the information "
                    ...
                    "e-commerce website" and "How many") in prompt:  # task 8 // might has to change
                prompt_edit = (("You will be given a product. Answer the given question based on the information "
            ...
            if (("xplain " and "category name") in prompt or ("xplain " and "product type") in prompt or (
            ...
            elif ("user" in prompt and "may also buy" in prompt and "product" in prompt):
                prompt_edit = (
                ...
                print("--------------------------------------------------------")
                print(("user" and "may also buy" and "product") in prompt)
                print("user" and "may also purchase" and "product" in prompt)
                print("--------------------------------------------------------")
                # print(("user" and ("may also buy" or "may also purchase") and "product") in prompt)
                print("user" in prompt)
                print("may also buy" in prompt)
                print("may also purchase" in prompt)
                print("product" in prompt)
                print(prompt)
                print("--------------------------------------------------------\n")
